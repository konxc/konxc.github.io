...
async function testSmartHeader() {
  ...
  try {
    for (const url of TEST_URLS) {
      ...
      await page.evaluate(() => {
        const blogPostHeader = document.querySelector(
      ...
      if (!showClasses || !showClasses.includes("hidden")) {
        ...
        console.log(
          "⚠️  Show behavior not working (hidden class still present)",
        );
      }
      ...
      if (!topClasses || !topClasses.includes("scrolled")) {
        ...
        console.log(
          "⚠️  Scroll behavior not working (scrolled class still present)",
        );
      }
