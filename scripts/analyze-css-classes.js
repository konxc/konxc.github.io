  ...
  async generateReport() {
    ...
    if (this.missingClasses.length > 0) {
      report.recommendations.push(
        "Some TOC classes are missing from build output",
      );
      ...
      if (
        this.missingClasses.some(
          (cls) => cls.includes("neutral") || cls.includes("primary"),
        )
      ) {
        report.recommendations.push(
      ...
      if (this.missingClasses.some((cls) => cls.startsWith("ml-"))) {
        report.recommendations.push(
          "Spacing classes missing - check spacing definitions",
        );
      }
    }
  ...
  async run() {
    try {
      ...
      if (this.missingClasses.length === 0) {
        ...
        console.log(
          `❌ ${this.missingClasses.length} classes are missing from build output`,
        );
        console.log(
          "💡 Check the recommendations in the report for next steps",
        );
      }
    } catch (error) {
