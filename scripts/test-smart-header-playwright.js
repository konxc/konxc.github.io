  ...
  try {
    ...
    if (!headerExists) {
      ...
      await takeScreenshot(
        page,
        `smart-header-not-found-${url.replace(/\//g, "-")}`,
      );
      return false;
    }
    ...
    if (!isVisible) {
      ...
      await takeScreenshot(
        page,
        `smart-header-hidden-${url.replace(/\//g, "-")}`,
      );
      return false;
    }
    ...
    if (headerStyles.position !== "fixed") {
      logResult(
        testName,
        false,
        `Header position is ${headerStyles.position}, expected 'fixed'`,
      );
      return false;
    }
    ...
    if (headerStyles.top !== "0px") {
      logResult(
        testName,
        false,
        `Header top is ${headerStyles.top}, expected '0px'`,
      );
      return false;
    }
    ...
    if (headerStyles.transform !== "matrix(1, 0, 0, 1, 0, 0)") {
      logResult(
        testName,
        false,
        `Header transform is ${headerStyles.transform}, expected no transform`,
      );
      return false;
    }
    ...
    await takeScreenshot(
      page,
      `smart-header-visible-${url.replace(/\//g, "-")}`,
    );
    return true;
  } catch (error) {
  ...
  try {
    ...
    if (!scrolledClasses || !scrolledClasses.includes("scrolled")) {
      logResult(
        testName,
        false,
        "Header does not have scrolled class after scrolling",
      );
      await takeScreenshot(
        page,
        `smart-header-scroll-failed-${url.replace(/\//g, "-")}`,
      );
      return false;
    }
    ...
    if (topClasses && topClasses.includes("scrolled")) {
      logResult(
        testName,
        false,
        "Header still has scrolled class after scrolling to top",
      );
      return false;
    }
    ...
    await takeScreenshot(
      page,
      `smart-header-scroll-success-${url.replace(/\//g, "-")}`,
    );
    return true;
  } catch (error) {
    logResult(testName, false, error.message);
    await takeScreenshot(
      page,
      `smart-header-scroll-error-${url.replace(/\//g, "-")}`,
    );
    return false;
  }
}
...
async function testSmartHeaderNavigation(page, url) {
  ...
  try {
    ...
    await takeScreenshot(
      page,
      `smart-header-nav-success-${url.replace(/\//g, "-")}`,
    );
    return true;
  } catch (error) {
    logResult(testName, false, error.message);
    await takeScreenshot(
      page,
      `smart-header-nav-error-${url.replace(/\//g, "-")}`,
    );
    return false;
  }
}
...
async function testSmartHeaderResponsive(page, url) {
  ...
  try {
    ...
    const mobileMenuToggle = await smartHeader
    ...
    await takeScreenshot(
      page,
      `smart-header-responsive-${url.replace(/\//g, "-")}`,
    );
    return true;
  } catch (error) {
    logResult(testName, false, error.message);
    await takeScreenshot(
      page,
      `smart-header-responsive-error-${url.replace(/\//g, "-")}`,
    );
    return false;
  }
}
