...
function getDiskUsage(path) {
  try {
    const result = execSync(`du -sh ${path} 2>/dev/null || echo "0B"`, {
      encoding: "utf8",
    });
...
async function comparePackageManagers() {
  ...
  runCommand(
    ...
    "Cleaning up npm artifacts",
  );

  // Install dependencies
  results.npm.install = runCommand(
    "npm install",
    "npm: Installing dependencies",
  );

  // Install Playwright
  results.npm.playwright = runCommand(
    "npm exec playwright install",
    "npm: Installing Playwright browsers",
  );

  // Run tests
  results.npm.tests = runCommand(
    "npm exec playwright test --reporter=list",
    "npm: Running tests",
  );
  ...
  runCommand(
    ...
    "Cleaning up pnpm artifacts",
  );

  // Install dependencies
  results.pnpm.install = runCommand(
    "pnpm install",
    "pnpm: Installing dependencies",
  );

  // Install Playwright
  results.pnpm.playwright = runCommand(
    "pnpm exec playwright install",
    "pnpm: Installing Playwright browsers",
  );

  // Run tests
  results.pnpm.tests = runCommand(
  ...
  const tasks = [
    {
      ...
      npm: results.npm.install,
      pnpm: results.pnpm.install,
    },
    {
      name: "Install Playwright",
      npm: results.npm.playwright,
      pnpm: results.pnpm.playwright,
    },
  ...
  tasks.forEach((task) => {
    if (task.npm && task.pnpm) {
      const improvement = (((task.npm - task.pnpm) / task.npm) * 100).toFixed(
        1,
      );
      ...
      log(
        `  ${faster} is ${Math.abs(improvement)}% faster`,
  ...
  const avgNpmTime =
    (parseFloat(results.npm.install) +
      parseFloat(results.npm.playwright) +
      parseFloat(results.npm.tests)) /
    3;
  const avgPnpmTime =
    (parseFloat(results.pnpm.install) +
      parseFloat(results.pnpm.playwright) +
      parseFloat(results.pnpm.tests)) /
    3;
