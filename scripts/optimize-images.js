...
async function optimizeImage(inputPath, outputPath, options) {
  try {
    ...
    console.log(
      `   Size: ${originalSize}KB → ${optimizedSize}KB (${reduction}% reduction)`,
    );
...
async function optimizeLogo() {
  ...
  if (!fs.existsSync(inputPath)) {
    ...
    console.error(
      "Please ensure logo-konxc.jpg exists in the public directory",
    );
    return;
  }
  ...
  for (const [sizeName, options] of Object.entries(CONFIG.sizes)) {
    console.log(
      `📐 Processing ${sizeName} size (${options.width}x${options.height})...`,
    );
  ...
  await optimizeImage(inputPath, mainWebpPath, {
    width: 64,
    height: 64,
    quality: 90,
  });
  ...
  console.log(
    `Files created: ${results.length * 2} (WebP + PNG for each size)`,
  );
