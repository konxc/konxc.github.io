...
export const PostContent = component$<PostContentProps>(({ content, id }) => {
  ...
  useVisibleTask$(async () => {
    ...
    processed = processed.replace(/\$\$([\s\S]+?)\$\$/g, (_match: string, math: string) => {
      try {
        const renderedMath = katex.default.renderToString(math.trim(), { displayMode: true, throwOnError: false, output: "html" });
        return `<div class="math-block">${renderedMath}</div>`;
      } catch { return `<div class="math-error"><code>${math}</code></div>`; }
    });

    processed = processed.replace(/(?<!\$)\$(?!\$)([^$\n]+?)(?<!\$)\$(?!\$)/g, (_match: string, math: string) => {
      try {
        return katex.default.renderToString(math.trim(), { displayMode: false, throwOnError: false, output: "html" });
      } catch { return `$${math}$`; }
    });
    ...
    const pendingNodes = document.querySelectorAll<HTMLElement>(`.mermaid-pending[data-post="${id}"]`);
  ...
  const ssrFallback = content.split(/\n\n+/).map((p: string) => `<p>${p.replace(/\n/g, "<br>")}</p>`).join("");
  return <div class="post-content" dangerouslySetInnerHTML={html.value || ssrFallback} />;
});
