...
export class NewsletterAnalytics {
  ...
  private updateConversionMetrics(data: ConversionData): void {
    try {
      ...
      metrics.conversions.bySource[source] =
        (metrics.conversions.bySource[source] || 0) + 1;

      // Update by variant
      const variant = data.variant;
      metrics.conversions.byVariant[variant] =
        (metrics.conversions.byVariant[variant] || 0) + 1;

      // Update by campaign
      if (data.campaignId) {
        const campaign = data.campaignId;
        metrics.conversions.byCampaign[campaign] =
          (metrics.conversions.byCampaign[campaign] || 0) + 1;
      }
...
if (typeof window !== "undefined") {
  ...
  const currentPageViews = parseInt(
    sessionStorage.getItem("page_views") || "0",
  );
